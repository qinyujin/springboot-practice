 ...
 * 或 | ：有1为1
 */
public class OneTimeInArr {
    public static void main(String[] args) {
        int[] arr = {2,4,3,6,3,2,5,5};
        int[] num1 = new int[arr.length];
        int[] num2 = new int[arr.length];
        OneTimeInArr oneTimeInArr = new OneTimeInArr();
        oneTimeInArr.FindNumsAppearOnce(arr,num1,num2);
        System.out.println();
    }
    ...
    public void FindNumsAppearOnce(int [] array,int num1[] , int num2[]) {
        ...
        //需要使用0来异或运算
        int diff = 0;
        //由于相同数据会抵消，因此实际上是4和6的异或运算即0100 ^ 0110 -> 0010
        for (int num : array) {
            diff^=num;
        }
